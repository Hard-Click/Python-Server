...
_meta_cache: dict[int, dict | None] = {}


def _meta_of(qid: int) -> dict | None:
    """question_id → Qdrant payload(courseId/sectionId/difficulty). 캐시한다."""
    if qid not in _meta_cache:
        _meta_cache[qid] = vector_store.retrieve_meta(qid)
    return _meta_cache[qid]
